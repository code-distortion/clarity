 ...
 */
class CatchTypeTest extends PHPUnitTestCase
{
     ...
     * @param boolean|null $expectedRethrow          The expected rethrow setting.
...
//                                                ->addCall('execute', $execute)
    ...
    private static function buildParams(MethodCalls $initMethodCalls): array
    {
        ...
        if (!$expectExceptionUponInit) {
            ...
            foreach ($initMethodCalls->getCalls() as $methodCall) {
                match ($methodCall->getMethod()) {
                    'catch' => $exceptnTypes = array_unique(array_merge($exceptnTypes, $methodCall->getArgsFlat())),
                    'match' => $matchStrings = array_unique(array_merge($matchStrings, $methodCall->getArgsFlat())),
                    'matchRegex' => $matchRgxs = array_unique(array_merge($matchRgxs, $methodCall->getArgsFlat())),
                    'known' => $knownStrings = array_unique(array_merge($knownStrings, $methodCall->getArgsFlat())),
                    'channel',
                    'channels' => $channels = array_unique(array_merge($channels, $methodCall->getArgsFlat())),
        ...
        return [
            ...
            'expectedExceptionClasses' => $exceptnTypes,
            'expectedMatchStrings' => $matchStrings,
            'expectedMatchRegexes' => $matchRgxs,
